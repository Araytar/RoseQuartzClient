...
import { WINDOW_NAMES, DISPLAY_OVERWOLF_HOOKS_LOGS, WINDOW_TITLE } from "app/shared/constants";
...
export const DesktopHeader = () => {
  ...
  return (
    <header className={"header"} onMouseDown={onDragStart} onMouseMove={onMouseMove}>
      <SVGComponent />
      <h1 className={"header__title"}>{WINDOW_TITLE}</h1>
      <div className={"header__controls__group"}>
        <button className={classNames("header__icon header__control header__discord")} onClick={handleDiscordClick}>
          <svg>
        ...
        <button
          className={classNames( "header__icon header__control header__toggle__icons", maximized ? "" : "header__toggled",)}
          onClick={toggleIcon}
        >
          <svg>
